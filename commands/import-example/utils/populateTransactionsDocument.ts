...
interface CreateFinancesDriveResponse {
  data: {
    addDrive: {
      id: string;
      name: string;
...
const CREATE_FINANCES_DRIVE_MUTATION = gql`
  mutation Mutation($preferredEditor: String, $global: DocumentDriveStateInput!) {
  addDrive(preferredEditor: $preferredEditor, global: $global) {
    id
    name
    }
  }
...
export const createFinancesDrive = async () => {
  try {
    const result = await fetchGraphQLWithRetry<CreateFinancesDriveResponse>(CREATE_FINANCES_DRIVE_MUTATION, {
      preferredEditor: "finances-drive",
      global: {
        name: "Finances",
        id: "finances"
      },
    ...
    return result.data.addDrive.id;
  } catch (error) {
    console.error('Error creating finances drive:', error);
    return null;
  }
};
